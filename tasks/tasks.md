...
| ~~EP-04~~ | ~~Structured Specification & Quality Engine~~ (v1: core engine + FE panels) | EP-01, EP-02, EP-03 | High | ✅ Done | ✅ Done | 📦 **Archived 2026-04-18** → `tasks/archive/2026-04-18-EP-04/` — 5 migrations + 9 dimension checkers (44 triangulated tests) + CompletenessService + NextStepDecisionTree + SectionService + ValidatorService + 5 endpoints wired + spec-gen Dundun callback + FE (SpecificationSectionsEditor, CompletenessPanel, NextStepHint, 2-col detail integration). 1174 BE tests green. v2 scope (bulk PATCH, POST /generate trigger, section history UI, ValidatorSuggestionEngine, QuickActionService wiring re-homed from EP-03, workspace_id defense-in-depth batch) = new follow-up epic |
...
| ~~EP-09~~ | ~~Listings, Dashboards, Search & Workspace~~ | EP-01, EP-02, EP-06, EP-08 | Medium-High | ✅ Done | ✅ Done | 📦 **Archived 2026-04-18** → `tasks/archive/2026-04-18-EP-09/` — all MVP surfaces shipped: BE (saved searches + list controllers + person/team dashboards + pipeline + kanban endpoints, `mine` filter) + FE (workspace dashboard, list + filters + search + QuickFilterChips + SavedFilterPresets + QuickViewPanel + SortControl + PipelineBoard + Kanban board + person/team dashboards + cursor Load more). Kanban closure this run: 27 new tests (card 11 + column 9 + hook 7) + real bug fix in `lib/api/kanban.ts` (cursor_${key} params were dropped on the wire → load-more was silently re-fetching page 1). Deferred v2 polish: group_by toolbar picker (URL param scaffolded), React Query adoption, useSearchParams wiring in SearchBar, scroll-position preservation |
...
| ~~**EP-13**~~ | ~~**Semantic Search + Puppet Integration** (v1: search + outbox)~~ | EP-09, EP-10, EP-12 | High | ✅ Done | ✅ Done | 📦 **Archived 2026-04-18** → `tasks/archive/2026-04-18-EP-13/` — v1 shipped (Puppet search wrapper + outbox ingest + Celery `process_puppet_ingest` + HMAC callback + admin observability + FE 59/61). v2 follow-up epic for doc-sources full ingestion (GitHub/URL scanners), reconciliation job, IndexingService hook, integration tests (blocked on Puppet upstream platform-ingestion endpoints) |
| ~~**EP-14**~~ | ~~**Hierarchy: Milestones, Epics, Stories** (v1: FK plumbing + FE UI)~~ | EP-01, EP-05, EP-09, EP-10 | High | ✅ Done | ✅ Done | 📦 **Archived 2026-04-18** → `tasks/archive/2026-04-18-EP-14/` — v1 shipped (migrations 0030/0031 + WorkItemType enum + parent_work_item_id threading + list direct-children filter + FE hierarchy UI 48 tests green). v2 follow-up epic for HierarchyValidator, materialized_path, cycle detection, CompletionRollupService, TreeQueryService, reparent + delete hierarchy ops. ⚠️ v2 MUST land before any PATCH-reparent endpoint goes live (data-corruption risk) |
...
| **EP-17** | **Edit Locking + Collaboration Control** | EP-01, EP-08, EP-10, EP-12 | Medium-High | ✅ 7 endpoints shipped (acquire/heartbeat/release/force-release + `GET /locks` list + unlock-request/respond) + force-release reason param (lock_controller:215) + list-embed lock summary (work_item_controller:421-442, 2 tests). Domain models + repos + mig 0119 + 8 tests. Service logic lives in controller (DDD smell — LockService extraction = refactor debt). | 🟡 30/55 (G0-G7 shipped with ~40 tests: types + useSectionLock + LockBadge + LockBanner + useRelativeTime + UnlockRequestDialog + HolderResponsePanel + ForceReleaseDialog). **Remaining gap**: G8 detail-page integration (~4-6h wiring lock lifecycle into edit-mode + SSE subscriptions + navigation guards); G9-G11 (list badges + loss recovery + a11y) scope-cut candidates | [x] Proposal [x] Specs [x] Design [x] Tasks [x] Backend [ ] Frontend — only G8 FE integration remains for MVP |
| ~~**EP-18**~~ | ~~**MCP Server: Read & Query Interface** (v1 read-only PoC)~~ | EP-00..EP-19 | Medium | ✅ Done | N/A | 📦 **Archived 2026-04-18** → `tasks/archive/2026-04-18-EP-18/` — v1 PoC shipped: 9/15 read-only tools end-to-end (~165 tests), `MCP_TOKEN` JWT auth + workspace isolation + `mcp>=1.26.0` SDK + `InMemoryCacheAdapter`. v2 follow-up epic for remaining 6 tools (Capability 4) + auth admin endpoints + FE token panel + rate-limit wiring. Read-only scope-cut explicit in proposal §Non-Goals |
