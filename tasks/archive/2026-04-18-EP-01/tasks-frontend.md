...
- **No dedicated `WorkItemList` component.** ~~The list is inlined into the page. Extraction is deferred.~~ **[x] FIXED 2026-04-17** — extracted `frontend/components/work-item/work-item-list.tsx` (props: `items, slug, isLoading?, error?, emptyState?`); 8 tests in `frontend/__tests__/components/work-item/work-item-list.test.tsx`; page reduced to filter toolbar + pagination + `<WorkItemList>`.
- **Table layout regression after `WorkItemCard` extraction.** ~~The page renders `<thead>` with 5 columns, then wraps the card in a single `<td colSpan={5}>`. Column alignment broken.~~ **[x] FIXED 2026-04-17** — replaced `<table>/<thead>/<tbody>` with `<ul role="list" aria-label={tItems('listAria')}>` / `<li>` per item; `WorkItemCard` gained `w-full`, lost `border-b` (handled by `divide-y` on list); added `workspace.items.listAria` to en/es locales; test updated from row-click-mockPush to link-href assertion.
- **State filter is not URL-synced.** ~~Only `?page=` reflected in URL.~~ **[x] FIXED 2026-04-17** — `stateFilter` reads initial value from `searchParams.get('state')`; `useEffect` syncs both `state` + `page` to URL on every change; filter change resets `page=1`; 3 new tests cover pre-select, change→URL, clear→no-state-param.
