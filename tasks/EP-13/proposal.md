...
> **⚠️ SUPERSEDES (2026-04-14, post-EP-18)**: after reviewing Puppet's real OpenAPI (v0.1.1), the workspace-isolation mechanism in this epic is updated. **Workspace scoping rides on Puppet `category`, not `tags`.** Convention: `tuio-wmp:ws:<workspace_id>:workitem|section|comment` (authoritative definition in `tasks/EP-18/specs/read-tools-assistant-search-extras/spec.md#semantic-search`). `tags` remain for per-entity facets (state, type, owner, team, archived, user-tag slugs). Any text below that says "tag `wm_<workspace_id>`" should be read as "category `tuio-wmp:ws:<workspace_id>:<kind>`"; `wm_state_*`, `wm_type_*`, etc. facet tags remain as tags (naming will be normalised during implementation). Puppet platform-ingestion endpoints are **pending upstream**; `semantic.search` and keyword search over workspace content are empty until those endpoints ship. External Tuio docs (under `tuio-docs:*` categories, Puppet's Notion pipeline) already work. Snippet highlighting is generated by our server (Puppet returns plain text only). Full string-level cleanup of this epic is deferred to the implementation PR; the spec in EP-18 is the authoritative source of truth.
