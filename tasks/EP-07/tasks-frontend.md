...
- [x] 6.3 [RED/GREEN] Comments tab comment count badge updates optimistically when a new comment is added (Group 6.3). New files: `frontend/components/work-item/comments-context.tsx` (`CommentsProvider` + `useCommentsContext`, single `useComments` instance shared across trigger and tab content) and `frontend/components/work-item/comment-count-badge.tsx` (reads from context, returns null when count === 0 or when no provider is present, counts only non-soft-deleted comments including nested replies). `CommentsTab` refactored into dual-mode (`CommentsTabFromContext` + `CommentsTabStandalone`) so existing isolated tests keep working without a provider. Detail page wraps the entire `<Tabs>` block in `<CommentsProvider workItemId={id}>` and `<TabsTrigger value="comentarios">` now renders `<CommentCountBadge />` alongside the label. Tests in `__tests__/components/work-item/comment-count-badge.test.tsx`: null-without-provider, count-from-context, zero-hides, deleted-not-counted, and the acceptance test "updates optimistically when a comment is added in CommentsTab" — 2026-04-18
...
- [x] 7.1 [RED] Test: diff viewer loading skeleton renders section placeholders — added in `__tests__/components/work-item/skeletons.test.tsx` (asserts `.animate-pulse` placeholder count >= 3) — 2026-04-18
