"""EP-13 — Async function for Puppet ingest pipeline.
...
Plain async function — no Celery. Triggered via:
  POST /api/v1/internal/jobs/process_puppet_ingest/run (superadmin)
Host cron hits that endpoint on the desired schedule.

_build_ingest_deps is monkeypatch-able for tests.
...
async def process_puppet_ingest(*, batch_limit: int = 50) -> dict[str, int]:
    ...
    try:
        ...
        logger.exception("puppet_ingest.error: %s", exc)
        if session is not None:
