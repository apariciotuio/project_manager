...
async def test_patch_tag_cross_workspace_returns_404(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws_b.id, created_by=user_b.id, name="cross-tag-patch"
    )
...
async def test_delete_tag_cross_workspace_returns_404(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws_b.id, created_by=user_b.id, name="cross-tag-delete"
    )
...
async def test_remove_work_item_tag_cross_workspace_returns_404(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws_b.id, created_by=user_b.id, name="cross-tag-rm"
    )
...
async def test_patch_tag_authorized_returns_200(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="original-name"
    )
...
async def test_delete_tag_authorized_returns_200(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="to-archive"
    )
...
async def test_remove_work_item_tag_authorized_returns_200(http, migrated_database) -> None:
    ...
    wi = await _create_work_item(migrated_database, workspace_id=ws.id, created_by=user.id)
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="remove-me-tag"
    )
...
async def test_patch_tag_name_only(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="patch-name-only"
    )
...
async def test_patch_tag_color_only(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="patch-color-only"
    )
...
async def test_patch_tag_archived_only(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="patch-archived-only"
    )
...
async def test_patch_tag_multi_field(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="patch-multi"
    )
...
async def test_patch_tag_all_none_returns_400(http, migrated_database) -> None:
    ...
    tag = await _create_tag(
        migrated_database, workspace_id=ws.id, created_by=user.id, name="patch-all-none"
    )
